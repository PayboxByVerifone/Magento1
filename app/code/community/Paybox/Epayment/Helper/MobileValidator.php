...
class Paybox_Epayment_Helper_MobileValidator {
	private $_Iso3166;

	public function __construct(){
		$this->_Iso3166 = Mage::helper('pbxep/Iso3166');
	}	
	...
	function normalize( $phone_number, $country_name = null, $onlynumber = false ) {
		...
		if( $country_name ) {
			...
			if ( $is_plus_prefixed ) {
				// comment originated from node-phone:
				// D is here.
				$prefix = $iso3166_entry["country_code"];
				if(strpos($phone_number,$prefix)===0){
					$phone_number = substr($phone_number,strlen($iso3166_entry["country_code"]));
				}else{
					$phone_number = "";	
				}	
			} else {
